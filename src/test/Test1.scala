...
//    println(Evaluator.printString(addVarVector))
   println(Evaluator.printString(DoubleEvaluator.eval( Map(i, If_Else(GreaterThan(VectorVarAccess(vectorVar,i),Drop(vectorVar,i)),IntLiteral(1) , IntLiteral(0)), Sequence((0 to vectorVar.len).toList))
   )))
